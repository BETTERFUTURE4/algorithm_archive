...
# 그런데 은상과 친구들은 다른 주전자의 막걸리가 섞이는 것이 싫어서, 분배 후 주전자에 막걸리가 조금 남아 있다면 그냥 막걸리를 버리기로 한다.  
...
while first <= last:
    ...
    if mid == 0:
        break
    if sum([(pot // mid) for pot in pots]) >= K:
        result = mid
        first = mid + 1
    else:
        last = mid - 1
...
# 최솟값은 0, 최댓값 맥스
# 최솟값이 최댓값보다 커질 때까지 반복
#     중간값 지정 : 최솟값 + 최댓값 / 2
#     중간값에서 k명에게 나눠줄 수 있는지 확인
#         못 나눠줄 시, 최댓값이 중간값이 됨
#         나눠줄 시 최솟값이 중간값이 됨
